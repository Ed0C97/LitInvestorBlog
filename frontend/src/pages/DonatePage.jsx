...
// ✅ MOBILE OTTIMIZZATO - PADDING AUMENTATI + CARD PIÙ PICCOLA
...
const DonationFormWrapper = ({
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-8">
      {children}
      <FadeInOnScroll>
        <div className="flex justify-center pt-[2rem] pb-[8rem]">
          <div className="text-center">
            <Button
              type="submit"
              disabled={
                loading || !amount || !termsAccepted || !donorInfo.email
              }
              variant="outline-yellow"
              size="lg"
              className="min-h-[48px]"
            >
              <Heart className="w-[1rem] h-[1rem] mr-[0.5rem]" />
              <span>
                {loading ? 'Processing...' : `Donate Now €${amount || '0'}`}
              </span>
            </Button>
...
const DonatePage = () => {
  ...
  const [paymentMethod, setPaymentMethod] = useState('white');
  ...
  const paymentButtons = [
    ...
    },
    {
      ...
      label: 'PayPal',
      icon: SiPaypal,
      selectedClass: 'selected-paypal',
    },
    {
      value: 'google',
      label: 'Google Pay',
      icon: SiGooglepay,
      selectedClass: 'selected-google',
    },
    {
      value: 'apple',
      label: 'Apple Pay',
      icon: SiApplepay,
      selectedClass: 'selected-apple',
    },
    {
      value: 'samsung',
      label: 'Samsung Pay',
      icon: SiSamsungpay,
      selectedClass: 'selected-samsung',
    },
  ];

  const handleAmountSelect = (amount) => {
    setActiveSelection(amount);
    if (amount === 'custom') {
      setCustomAmount('');
    } else {
  ...
  useEffect(() => {
    if (user && user.email) {
      setDonorInfo((prevInfo) => ({
        ...prevInfo,
        email: user.email,
        name: user.username || user.displayName || prevInfo.username,
      }));
    }
  }, [user]);
  ...
  return (
    <div className="overflow-x-hidden bg-antracite">
      <FadeInOnScroll>
        <div className="w-full mb-[3rem]">
          {/* PADDING LATERALI: 2rem su mobile */}
          <div className="max-w-[1012px] mx-auto px-[2rem] sm:px-[16px] lg:px-[16px] pt-[3rem] smooth-padding">
            <div className="mb-[2rem]">
              <div className="border-b border-input-gray my-[0.5rem]"></div>
              <h2 className="text-xl sm:text-2xl font-regular text-white">
                Support with a ...
              </h2>
            </div>
            <p className="text-light-gray space-y-2 my-[2rem] sm:my-[3rem] text-justify text-sm sm:text-base">
              Your support helps us keep Lit Investor Blog free from intrusive
              ads and focused on delivering high-quality investment insights.
              With your help, we can continue to publish clear analysis,
              practical guides, and timely updates. Every contribution truly
              makes a difference.
            </p>
          </div>
        </div>
      ...
      {/* PADDING LATERALI: 2rem su mobile */}
      <div className="max-w-[1012px] mx-auto px-[2rem] sm:px-[16px] lg:px-[16px] pb-[4rem] smooth-padding">
        <div>
          <Card className="shadow-none border-none bg-transparent">
            <CardContent className="p-[0rem]">
              <Elements stripe={stripePromise}>
                <DonationFormWrapper
                  ...
                  paymentMethod={paymentMethod}
                >
                  <FadeInOnScroll delay={50} className="mb-[3rem]">
                    <DonationsTable 
                      variant="public" 
                      collapsible={true} 
                      limit={5} 
                  ...
                  <FadeInOnScroll delay={100} className="mb-[3rem] sm:mb-[5rem]">
                    <div className="space-y-4">
                      <div>
                        <div className="flex items-center gap-[0.75rem] text-white font-bold text-xl sm:text-2xl">
                          <Coins size={24} />
                          Select an amount
                        </div>
                        <p className="text-sm text-gray mt-[1.25rem] mb-[3rem] uppercase">
                  ...
                  <FadeInOnScroll delay={200}>
                    <div className="mt-[2rem] sm:mt-[3rem] lg:mt-[4rem]">
                      {/* Titolo con icona - mobile e desktop */}
                      <div className="flex items-center gap-[0.75rem] text-white font-bold text-xl sm:text-2xl">
                        <WalletCards size={20} className="lg:w-[24px] lg:h-[24px]" />
                        <span className="lg:text-2xl">Payment methods available</span>
                      </div>
                      <p className="text-sm text-gray uppercase mt-[1rem] mb-[4rem] lg:mb-[6rem]">
                        Enter your optional details or customise your amount
                      </p>
                      ...
                      <div className="w-full flex justify-center">
                        <GlassCardForm
                          ...
                          donorInfo={donorInfo}
                          onDonorInfoChange={handleInputChange}
                  ...
                  <FadeInOnScroll delay={300}>
                    <div className={`space-y-2 mt-[4rem] lg:mt-[3rem] ${donorInfo.message ? 'lg:pt-[6rem]' : 'lg:pt-[10rem]'}`}>
                      {donorInfo.message && (
                        <div className="text-white mb-[2rem] lg:mb-[2.5rem]">
                          <strong className="font-bold block mb-[1rem]">Your message</strong>
                          <p className="whitespace-pre-wrap break-words text-justify text-white/80">
                            {donorInfo.message}
                          </p>
                        </div>
                      )}
                      <div className="space-y-6 lg:pt-[2rem]">
                        <Checkbox
                          id="anonymous"
                          name="anonymous"
                          checked={donorInfo.anonymous}
                          onCheckedChange={(checked) =>
                            setDonorInfo((prev) => ({ ...prev, anonymous: checked }))
                          }
                          label="I want to make an anonymous donation"
                          description="If selected, your name will not appear publicly. Your email is never stored."
                        />
                        ...
                        <Checkbox
                          ...
                          label={
                            <span>
                              I accept the{' '}
                              <a href="/termini-e-condizioni" target="_blank" className="underline hover:text-white">
                                Terms and Conditions
                              </a>
                            </span>
                          }
                          description="By clicking, you confirm that you have read and accepted our terms of service."
                        />
                      </div>
                    </div>
                  </FadeInOnScroll>
