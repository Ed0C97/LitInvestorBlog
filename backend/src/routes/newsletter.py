    ...
    try:
        ...
        if existing:
            if existing.is_active:
                ...
                db.session.commit()
                return (
                    jsonify({"message": "Sottoscrizione riattivata con successo"}),
                    200,
                )

        subscriber = NewsletterSubscriber(
            email=email, preferences=data.get("preferences", {})
        )
        ...
        success, message = email_service.send_newsletter_confirmation(email)
        if not success:
            print(f"Errore invio email: {message}")

        return (
            jsonify({"message": "Iscrizione alla newsletter completata con successo"}),
            201,
        )
    ...
    try:
        ...
        return (
            jsonify(
                {
                    "subscribers": [
                        subscriber.to_dict() for subscriber in subscribers.items
                    ],
...
def create_donation():
    try:
        ...
        if not data.get("amount") or not data.get("payment_method"):
            return (
                jsonify({"error": "Importo e metodo di pagamento sono obbligatori"}),
                400,
            )
        ...
        donation.payment_id = (
            f"sim_{donation.id}_{int(donation.created_at.timestamp())}"
        )
        db.session.commit()
...
def get_notification_preferences():
    try:
        ...
        return jsonify({"preferences": [pref.to_dict() for pref in preferences]}), 200
...
def set_notification_preference():
    try:
        ...
        return (
            jsonify({"message": "Preferenze di notifica aggiornate con successo"}),
            200,
        )
