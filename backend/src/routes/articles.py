...
from src.utils.file_helpers import delete_image_file
...
def get_articles():
    try:
        ...
        # Leggi tutti i parametri in una volta
        ...
        # Inizia sempre da tutti gli articoli
        query = Article.query

        # Se NON siamo nell'admin page, applica il filtro di default per i soli pubblicati
        if not include_all:
            query = query.filter(Article.published.is_(True))

        # Ora applica i filtri specifici, uno per uno
        if status:
            now = datetime.utcnow()
            if status == 'published':
                query = query.filter(Article.published.is_(True),
                ...
                query = query.filter(Article.published.is_(True), Article.published_at > now)
            elif status == 'draft':
                query = query.filter(Article.published.is_(False))
    ...
    try:
        ...
        if "category_id" in data:
            ...
            if "published_at" in data:
                article.published_at = parse_date(data.get("published_at"))
            if "show_author_contacts" in data:
                article.show_author_contacts = data["show_author_contacts"]

            images_to_delete = data.get('images_to_delete')  # Il frontend ci invierà questa lista
            if isinstance(images_to_delete, list):
                for image_url in images_to_delete:
                    # Per ogni URL nella lista, chiama la nostra funzione di cancellazione
